...
func build_hud() -> void:
	...
	xp_bar.size=Vector2(1232,12)
	xp_bar.show_percentage=false
	root.add_child(xp_bar)
	var hint:=Label.new()
	hint.text="WASD / arrows to move • P to pause • touch left side to steer"
...
func spawn_enemy(kind: String) -> void:
	if live_enemies().size() > 450 and kind not in ["elite_null", "the_silence"]:
		return
	var enemy:=ForceEnemy.new()
	add_child(enemy)
	var a:=rng.randf_range(0,TAU)
	var dist:=rng.randf_range(520,700)
...
func show_level_choices()->void:
	...
	var title:=Label.new()
	title.text="SIGNAL ACQUIRED — CHOOSE AN UPGRADE"
	...
	for option in options:
		...
		button.text="%s  •  Rank %d\n%s" % [data.name,option.level,data.desc]
		button.custom_minimum_size=Vector2(640,82)
		button.pressed.connect(choose_upgrade.bind(option.id,option.passive))
...
func _unhandled_input(event:InputEvent)->void:
	if event is InputEventKey and event.pressed and event.keycode == KEY_P and not ended:
...
func _draw()->void:
	...
	for x in range(-1500,1501,100):
		draw_line(Vector2(x,-1000),Vector2(x,1000),Color(0.15,0.32,0.48,0.14),1.0)
	for y in range(-1000,1001,100):
		draw_line(Vector2(-1500,y),Vector2(1500,y),Color(0.15,0.32,0.48,0.14),1.0)
	for m in mines:
