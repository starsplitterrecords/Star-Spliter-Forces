...
func show_title()->void:
	clear_screen()
	add_heading("STAR SPLITTER FORCES",82,54)
	var sub: Label = add_heading("CHARACTERS. WORLDS. MUSICAL PHYSICS.",150,18)
	sub.modulate = Color("68ecff")
	add_button("ENTER THE FIELD",250,show_character_select)
	add_button("QUICK TEST — 90 SEC",320,start_quick_test)
	add_button("SETTINGS",390,show_settings)
	var thesis: Label = Label.new()
	thesis.text = "Current showcase: Ilyra Venn / Rex Fleet / Pulse Width Codex Force"
	thesis.position = Vector2(80,466)
	thesis.size = Vector2(1120,32)
	thesis.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	thesis.modulate = Color(1,1,1,0.62)
	ui.add_child(thesis)
	var stats: Label = Label.new()
	stats.text = "RUNS %d    •    WINS %d    •    SIGNAL %d" % [int(save.runs),int(save.wins),int(save.currency)]
	stats.position = Vector2(80,510)
	stats.size = Vector2(1120,40)
	stats.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	stats.modulate = Color(1,1,1,0.45)
	ui.add_child(stats)
	var footer: Label = Label.new()
	footer.text = "Performance moves between RESONANCE / DRIFT / FRACTURE — the music never stops."
...
func show_character_select()->void:
	clear_screen()
	add_heading("SELECT A CHARACTER",55,42)
	var context: Label = add_heading("Forces are musical combat languages. Characters belong to the worlds.",112,17)
	context.modulate = Color(1,1,1,0.55)
	var unlocked: Array = save.unlocked_characters
	var ids: Array = GameData.CHARACTERS.keys()
	var y: float = 175.0
	for id in ids:
		...
		var lock_text: String = "" if unlocked_now else "  [WIN ONCE TO UNLOCK]"
		b.text = "%s — %s\n%s%s" % [d.name,d.origin,d.tagline,lock_text]
		b.add_theme_font_size_override("font_size",20)
		ui.add_child(b)
		if unlocked_now:
...
func start_run(id:String, quick_test:bool=false)->void:
	...
	current_run.setup(id, quick_test)
...
func show_results(result:Dictionary)->void:
	clear_screen()
	add_heading("SIGNAL STABILIZED" if result.victory else "SIGNAL LOST",90,48)
	var summary: Label = Label.new()
	summary.text = "%s\nTIME  %s\nLEVEL  %d\nPURGED  %d\nFINAL STATE  %s (%d%%)\n\n+%d SIGNAL" % [GameData.CHARACTERS[result.character].name,GameData.format_time(result.time),int(result.level),int(result.kills),String(result.state),int(result.resonance),int(result.kills/5)+int(result.level)*2+(100 if result.victory else 0)]
	summary.position = Vector2(350,190)
	summary.size = Vector2(580,270)
	summary.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	summary.add_theme_font_size_override("font_size",21)
	ui.add_child(summary)
	add_button("RUN AGAIN",500,start_run.bind(String(result.character), false))
	add_button("RETURN TO ARRAY",570,show_title)

func show_settings()->void:
	clear_screen()
	add_heading("SETTINGS",90,42)
	var music: HSlider = HSlider.new()
	music.min_value=0; music.max_value=1; music.step=0.05; music.value=float(save.settings.music); music.position=Vector2(420,230); music.size=Vector2(440,40); ui.add_child(music)
