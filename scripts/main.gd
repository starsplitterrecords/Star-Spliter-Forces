...
func clear_screen() -> void:
	...
	var bg:=ColorRect.new(); bg.color=Color("080914"); bg.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT); ui.add_child(bg); bg.mouse_filter=Control.MOUSE_FILTER_IGNORE

func add_heading(text:String, y:float, size:int=48)->Label:
	var label:=Label.new(); label.text=text; label.position=Vector2(80,y); label.size=Vector2(1120,80); label.horizontal_alignment=HORIZONTAL_ALIGNMENT_CENTER; label.add_theme_font_size_override("font_size",size); label.modulate=Color("dffcff"); ui.add_child(label); return label
...
func on_run_finished(result:Dictionary)->void:
	...
	save.runs=int(save.runs)+1; save.best_time=maxf(float(save.best_time),float(result.time)); save.currency=int(save.currency)+int(result.kills/5)+int(result.level)*2
	if result.victory:
		save.wins=int(save.wins)+1; save.currency=int(save.currency)+100
...
func show_settings()->void:
	...
	var music:=HSlider.new(); music.min_value=0; music.max_value=1; music.step=0.05; music.value=float(save.settings.music); music.position=Vector2(420,230); music.size=Vector2(440,40); ui.add_child(music)
	var ml:=Label.new(); ml.text="MUSIC INTENSITY"; ml.position=Vector2(420,195); ui.add_child(ml)
	var sfx:=HSlider.new(); sfx.min_value=0;sfx.max_value=1;sfx.step=0.05;sfx.value=float(save.settings.sfx);sfx.position=Vector2(420,330);sfx.size=Vector2(440,40);ui.add_child(sfx)
	var sl:=Label.new();sl.text="SFX LEVEL";sl.position=Vector2(420,295);ui.add_child(sl)
	var shake:=CheckButton.new();shake.text="SCREEN SHAKE";shake.button_pressed=bool(save.settings.screen_shake);shake.position=Vector2(420,395);ui.add_child(shake)
