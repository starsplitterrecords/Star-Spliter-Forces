...
const SIGNAL_POSITIONS: Array = [Vector2(1850,-920),Vector2(-2700,-1550),Vector2(3300,1900)]
...
class ExpeditionOverlay:
    ...
    var extraction_point: Vector2
    var landmarks: Array
    func _init(run_node: GameRun, size_value: Vector2, extraction_value: Vector2, landmark_values: Array) -> void:
        run = run_node
    ...
    func _draw() -> void:
        ...
        var ready: bool = run.extraction_ready
...
func _process(_delta: float) -> void:
    ...
    if root == null:
        return
    var run: GameRun = _find_run(root)
    if run == null:
        return
...
func _find_run(node: Node) -> GameRun:
    if node is GameRun:
        return node as GameRun
    for child in node.get_children():
        var found: GameRun = _find_run(child)
        if found != null:
            return found
    return null

func _configure_run(run: GameRun) -> void:
    if not run.quick_test_mode:
    ...
    for node in run.discovery_nodes:
        if is_instance_valid(node) and index < SIGNAL_POSITIONS.size():
            ...
            index += 1
    var overlay: ExpeditionOverlay = ExpeditionOverlay.new(run, HALF_SIZE, EXTRACTION_POINT, LANDMARKS)
    overlay.z_index = -10
...
func _update_run(run: GameRun) -> void:
    if run.player == null:
        return
    if not run.quick_test_mode and run.extraction_ready and not run.boss_spawned:
        if run.player.position.distance_to(EXTRACTION_POINT) <= 180.0:
            run.spawn_boss()
    var guide: Label = run.hud_layer.get_node_or_null("ExplorationGuide") as Label
    if guide != null:
        guide.text = _guidance_text(run)

func _guidance_text(run: GameRun) -> String:
    if run.boss_spawned:
        return "ARRAY GATE: extraction contested"
    if run.extraction_ready:
    ...
    for node in run.discovery_nodes:
        if is_instance_valid(node):
            var distance: float = run.player.position.distance_to(node.position)
